...
Operational visibility and rollout tooling. Addresses [#7](https://github.com/fruteroclub/intern-os/issues/7): active workstream registry and rollout protocol for production internOS.

### New features

- **Active workstream registry** — `projects/REGISTRY.md` is a derived index of all workstreams generated by `generate-registry.sh`. Contains project, workstream name, thread_id, phase, owner, health status, and filesystem path. Explicitly derived from BRIEF.md + STATUS.md — never authoritative. Lives in `projects/` (not workspace root) to avoid auto-loading on every agent message.
...
- `intern-os/scripts/generate-registry.sh` — **new**: active workstream registry generator
