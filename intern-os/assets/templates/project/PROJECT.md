...
status: [discovery | active | blocked | closing | archived]
current_phase:
next_milestone:
blockers:
...
Default behavior (with these fields absent or `shared_thread_ids: false`):
each workstream must have a unique thread_id. Duplicates produce a WARN.
...
<!-- Updated when workstreams are activated or archived -->
